...
2. 세 시간규모의 신호를 결합함. 장기(outcome)는 기존의 궤적 끝 성공/실패. 단기(feedback)는 직전 행동의 즉각 응답 — "Nothing happens"면 -1, "You pick up ..."이면 +1. 중기(state-history)는 최근 K=5턴에서 부정 피드백 반복이면 -1, 긍정이 한 번도 없으면 -η. 이걸 결합해 스텝별 가중치를 만들어 기존 리턴에 곱함.
...
5. 결과. ALFWorld 95.31(1.5B), 96.03(7B), WebShop Task Score 89.81로 HGPO, GiGPO, GraphGPO 전부 넘김. 특히 WebShop의 Task Score는 구매 성공 여부가 아니라 요구사항 일치도를 재는 것 — 크레딧 할당이 의미론적 적합성까지 개선한다는 뜻.
