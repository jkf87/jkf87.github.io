...
2. Tycho(게임 규칙을 실행 가능한 파이썬 프로그램으로 써서 그 안에서 계획을 세우는 에이전트 시스템)의 접근은 게임 규칙을 파이썬 프로그램으로 직접 쓰는 것임. init_state, transition, render, outcome 네 함수를 구현하는데 객체 목록이든 상태 기계든 셀룰러 오토마타든 에이전트가 형식을 자유롭게 고름. 사전 정의 분해가 없다는 게 유연성의 원천임.
