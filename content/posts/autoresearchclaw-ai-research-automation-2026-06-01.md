...
1. 기존 시스템의 빈틈 세 가지 진단이 정확함. 단일 에이전트 중심 추론(가설 세우고 비판하고 수정까지 혼자 다 — 검토자가 없으니 자기 오류를 걸러낼 단계가 없음), 실패 시 중단(실패를 정보가 아니라 종료 조건으로 처리), 실행 간 기억 부재(어제 교훈이 오늘에 안 전달됨). 세 항목 모두 학습이 시스템에 축적되지 않는다는 같은 결함임.
...
3. 장치 2, 자가 복구 실행. 실패 시 멈추지 않고 실패 로그를 분석해 PROCEED(계속)/REFINE(수정 재시도)/PIVOT(방향 전환) 중 하나를 결정. 목표는 유지한 채 도달 경로만 다시 계산하는 절차를 시스템화한 것 — EnvRigger의 취약점 진단 후 설정 탐색과 같은 발상.
...
5. 장치 4, Human-in-the-Loop(사람이 중간에 개입하는 방식)이 제일 흥미로움. CoPilot 모드가 평균 19회 개입으로 품질 7.27, accept rate 87.5%. 모든 단계 승인받는 Step-by-Step은 29회 개입에도 품질 5.19, accept 50%. 더 많이 끼어들수록 좋은 게 아니라 고레버리지 순간에 정확히 들어가는 게 핵심이라는 것.
...
9. 여기서 문제제기. 이 결과의 전제를 봐야 함. 벤치마크가 저자들이 구성한 ARC-Bench(자율 연구 파이프라인 결과를 채점하는 벤치마크)와 scoring rubric에 의존 — 외부 독립 재현이 아직 없음. 논문 실험은 25-topic, 공개 벤치마크는 55-topic으로 표기가 갈려서 수치 인용 시 구분 필요. HITL ablation도 10개 topic 샘플. Full-Auto도 완료율은 10/10이지만 품질 5.62, accept 3/10 — "완전 자동 연구 = 고품질"로 과장하면 안 됨.
