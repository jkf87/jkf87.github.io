...
<span style="background-color: #fff59d"><strong>PARL 계열은 거의 꽉 찹니다</strong></span>. PARL-A는 Amazon `99.3%`, Reddit `99.9%`, News `99.0%`입니다. PARL-B는 Amazon `99.5%`, Reddit `99.4%`, News `100.0%`입니다.
...
이 대목이 중요합니다. 사용자 히스토리를 프롬프트에 넣는다고 자동으로 개인화 평가가 되는 건 아닙니다. <span style="background-color: #fff59d"><strong>Judge에게 히스토리를 보여주는 것과, Judge가 쓸 기준을 학습시키는 것은 다릅니다</strong></span>.
...
그래서 이 논문의 메시지는 꽤 실무적입니다. 개인화 시스템을 만들고 싶다면 <span style="background-color: #fff59d"><strong>생성기보다 평가기부터 개인화해야 합니다</strong></span>.
