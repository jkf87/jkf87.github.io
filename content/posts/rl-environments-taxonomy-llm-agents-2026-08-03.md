...
4. 하네스는 모델이 환경과 만나는 방식임. rollout 프로토콜, 도구, 시스템 프롬프트, 컨텍스트 관리, 샌드박스를 통제하지만 모델의 지식을 늘리는 요소는 아님. 도구의 deterministic/stateful 여부가 재현성과 크레딧 배분에 영향을 주고, 최근 하네스들이 도구 수를 다시 줄여 read/write/edit/bash/subagent 수준으로 수렴한다는 관찰도 실무 감각과 일치함. 600턴 리서치 에피소드를 위한 컨텍스트 전략 네 가지(최근 유지, 마르코프 재구성(과거를 현재 상태 하나로 요약해 다음 행동이 그 요약만으로 결정되게 만드는 재구성), 인용 보존 요약, 인용 보존 폴딩)도 그대로 챙길 목록임.
...
7. 제일 쓸모 있는 대응 관계는 "벤치마크는 고정된 RL 환경"임. 벤치마크의 요청-환경-종료조건-채점기 구조가 RL 환경 구성요소와 거의 대응되고, 차이는 freeze 여부뿐. 좋은 벤치마크 원칙이 훈련 환경에도 그대로 감 — 자연스러운 태스크, 자동 검증 가능한 채점, 난이도 캘리브레이션, 그리고 채점기 독립성(생성 모델과 같은 패밀리의 judge를 쓰면 보상 해킹이 생김).

8. 필자 관점. 이 분해는 필자가 계속 정리해온 결과들과 그대로 연결됨. DocsChisel의 "도구 설명이 곧 프롬프트"가 H, RuleMem·Magenta의 검증 설계가 V, SearchOS의 상태 외부화가 S에 대응됨. 환경 생태계가 패키지와 매니지드 API로 간다는 전망(환경 허브, ORS 프로토콜)도 MCP(LLM이 외부 도구·데이터를 표준 규격으로 연결하는 프로토콜)가 도구 통합에서 했던 역할을 RL 프리미티브에서 반복한다는 방향이라 수긍됨.
