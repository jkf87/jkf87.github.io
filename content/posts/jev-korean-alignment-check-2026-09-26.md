...
date: 2026-09-26
author: 한준구(코난쌤)
description: "AI 응답이 사실을 굽혔는지, 자료에 없는 말을 지어냈는지, 문서에 숨은 지시를 따랐는지 검사기로 거르는 방법. 논문(arXiv 2609.29429)을 한국어 교실·업무 사례 48문항으로 직접 재현해 AUROC와 문턱값을 실측했습니다."
retracted_reason: "논문의 핵심 기여(문턱값 0.5 과소발화와 10건 라벨로 문턱 보정)를 자체 발견으로 서술함. 저자 코드·본문 확인 후 재작성 필요."
tags:
  - AI-검증
  - 업무자동화
  - 교사
...
교실과 사무실에서 실제로 겪는 장면으로 옮기면 이렇습니다.

| 실패 유형 | 겪는 장면 | 왜 눈에 안 띄나 |
|---|---|---|
...
- 실패 유형을 3가지만 봤습니다. 논문은 10가지입니다. 탈옥이나 권력 추구처럼 교실·사무실과 먼 유형은 뺐습니다.
...
학교 업무로 옮기면 이렇게 됩니다. 민원 메일 요약, 회의록 정리, 생활기록부 문구 다듬기처럼 <span style="background-color: #fff59d"><strong>원자료가 분명히 있는 작업</strong></span>에 이 검사가 붙습니다. AI가 정리한 결과와 원자료를 같이 넣고, 확률이 높은 것만 사람이 다시 봅니다. 자료 없이 새로 쓰는 글에는 쓸 수 없습니다. 대조할 근거가 없어서입니다.
