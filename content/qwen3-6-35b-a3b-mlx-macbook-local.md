...
```
4bit 양자화 모델 다운로드: ~20GB (safetensors)
런타임 VRAM 예상:        ~18-22GB (모델 + KV 캐시)
M4 32GB 여유 메모리:     ~10-14GB
```
...
실무에서는 **8K-32K 컨텍스트**를 유지하면 안정적으로 사용할 수 있다.
...
### 맥북 M4 16GB에서도 돌아가나요?
16GB에서는 모델 로딩 자체가 어려울 수 있습니다 (~20GB 필요). 최소 **24GB, 권장 32GB**입니다.
