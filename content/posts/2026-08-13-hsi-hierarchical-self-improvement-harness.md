...
3. 결과. BabyAI 42.0→81.3(+39.3), Crafter(광물 채집·제작·생존으로 구성된 게임 환경 벤치마크) 11.6→44.6(+33.0), TextWorld(텍스트 기반 어드벤처 게임으로 지시 이해력을 측정하는 벤치마크) 40.0→65.0(+25.0), MiniHack(미니게임 형태의 강화학습 환경 모음) 0.8→15.8임. TextWorld 65.0%는 Grok-4(62.9%)와 Claude-Opus-4.5-Thinking(59.0%)을 넘는 수치임. 같은 모델, 같은 추론 예산에서 하네스만 바꿔서 얻은 결과라는 게 의미임.
...
9. 문제제기. BALROG(텍스트 게임 6종으로 에이전트의 탐색·계획·장기 추론 능력을 측정하는 벤치마크)가 텍스트 게임 6종이라 실무 도메인 검증은 아니고, outer iteration 5회·80스텝 설정의 비용이 환경마다 어떻게 달라지는지도 불투명함. 그래도 "동결 모델 + 하네스 진화"로 프론티어급 점수를 낸 사례 자체가 방향성을 보여줌.
