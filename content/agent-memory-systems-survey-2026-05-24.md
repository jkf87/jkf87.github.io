---
title: "LLM 에이전트 메모리 시스템 비교: RAG·그래프·장기기억 벤치마크 정리"
date: 2026-05-24
tags:
  - ai-agents
  - memory-systems
...
LLM 에이전트가 실무에서 오래 쓰이려면 긴 컨텍스트만으로는 부족합니다. 사용자 선호, 과거 결정, 시간 순서, 지식 업데이트를 다루려면 RAG, 지식그래프, 계층형 메모리, 선택적 기록 정책을 함께 설계해야 합니다.

원문·참고: [LoCoMo arXiv 2402.17753](https://arxiv.org/abs/2402.17753), [LongMemEval arXiv 2410.10813](https://arxiv.org/abs/2410.10813)

| 분류 | 대표 방식 | 강점 |
|---|---|---|
| RAG 기반 | Mem0, A-MEM, MemoRAG | 필요한 기억을 검색해 컨텍스트에 주입 |
| 그래프 기반 | Graphiti, HippoRAG | 사람·사건·시간 관계 추적 |
| 계층형 | MemGPT, MemoryOS | 작업 메모리와 장기 저장소 분리 |
| 프로덕션 | Codex, Copilot, Claude Code, OpenClaw | 실제 개발 흐름과 파일 기반 기억 결합 |
...
## FAQ: LLM 에이전트 메모리 검색 질문

### LLM 에이전트 메모리는 RAG와 같은 건가요?

RAG는 중요한 구성 요소이지만 전부는 아닙니다. 장기 대화, 시간 추론, 사용자 선호 업데이트까지 다루려면 기록·정리·검색·폐기 정책이 함께 필요합니다.

### LoCoMo와 LongMemEval은 무엇을 평가하나요?

LoCoMo는 장기 대화 기억을, LongMemEval은 채팅 어시스턴트의 장기 상호작용 기억을 평가하는 벤치마크입니다.

### 실무에서는 어떤 메모리 구조가 가장 현실적인가요?

파일 기반 로그와 요약 메모리, 검색 가능한 지식 저장소, 명시적 사용자 선호를 분리해 관리하는 하이브리드 구조가 가장 현실적입니다.
