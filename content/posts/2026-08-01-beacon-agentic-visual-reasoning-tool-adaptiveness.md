...
4. Tool Effect는 실제 성능 영향을 정량화함. text-hard에서 도구로 맞춘 비율이 Tool-Gain, text-easy에서 도구 쓰다가 틀린 비율이 Tool-Harm임. 기존 모델들의 ΔTE는 거의 0이었음. 전체 정확도가 올라가 보였던 건 SFT(지도 미세조정, 정답 예시를 보여주며 가르치는 사전학습 단계) 데이터 효과지 도구 사용 자체의 효과가 아니었다는 게 밝혀진 것임.

![MA와 TE 지표](/images/2026-08-01-beacon-agentic-visual-reasoning-tool-adaptiveness/fig-3-p5.png)

5. 해법 첫 조각은 Necessity-Aware Adaptive Reward임. 기존 GRPO(같은 프롬프트에서 뽑은 여러 응답을 서로 비교해 상대적 우열로 학습하는 강화학습 알고리즘)는 정답이면 1, 오답이면 0이라 "항상 도구 쓰기"가 안전해 보임. NAAR는 텍스트로 풀리는 문제에선 텍스트 정답에 1.0, 도구 사용 정답에 0.25를 줌. 텍스트로 못 푸는 문제에선 도구 정답에 1.0임.
...
7. 두 번째 조각은 Hint-Guided Capability Expansion임. RLVR(정답 여부 같은 검증 가능한 보상만으로 모델을 강화학습하는 방식)의 근본 한계는 정책이 처음부터 못 푸는 문제는 학습 신호가 없다는 것. 전 롤아웃이 틀리면 그룹 내 어드밴티지가 무의미함.
