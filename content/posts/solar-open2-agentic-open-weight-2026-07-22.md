...
그런데 이 모델을 “한국어 잘하는 오픈웨이트 LLM” 정도로만 읽으면 핵심을 놓친다. Upstage가 이번에 앞세운 단어는 chat이 아니라 **agent**다. 긴 문서를 읽고, 도구를 부르고, 코드를 고치고, 엑셀·문서·프레젠테이션 같은 deliverable을 만들어내는 모델. Solar Open 2는 그 방향으로 설계된 한국어권 sovereign foundation model이다.

공식 모델카드, Upstage 블로그, 34쪽짜리 [Solar Open 2 Technical Report](https://huggingface.co/upstage/Solar-Open2-250B/blob/main/Solar_Open_2_Tech_Report.pdf), 그리고 조코딩 채널의 [Solar Open Weight Day 라이브](https://www.youtube.com/live/6XX-yR3qomM) 자막을 같이 읽으면 그림이 더 또렷해진다. 특히 6분 23초부터 이어지는 Upstage 이활석 CTO 설명은 테크니컬 리포트의 설계 선택을 “왜 그렇게 만들었나”의 언어로 바꿔준다. 이 모델의 재미있는 지점은 벤치마크 숫자 하나가 아니다. **아키텍처, 데이터, post-training, agent scenario가 모두 “긴 작업을 끝까지 수행하는 모델” 쪽으로 맞물려 있다.**
...
이 설계의 의도는 명확하다. 에이전트는 단답형 chat보다 훨씬 많은 토큰을 쓴다. 계획하고, 도구를 부르고, 결과를 읽고, 다시 수정하고, 파일을 열고, 검증한다. 모델 호출이 한 번으로 끝나지 않는다. 기업이 자체 인프라에서 agent를 돌리려면 “큰 모델의 능력”과 “작은 모델에 가까운 추론 비용”을 동시에 노려야 한다.
...
이활석 CTO는 라이브에서 이 선택을 더 직설적으로 설명했다. Solar Open 1은 all-softmax attention이었고, Solar Open 2는 softmax attention과 linear attention을 섞었다. 리포트 기준 구조는 3개의 linear attention마다 1개의 softmax attention이 들어가는 패턴이다. 이유는 단순하다. softmax attention은 긴 범위의 recall에 강하지만, context가 길어질수록 KV cache와 연산량이 커진다. linear attention은 긴 sequence를 더 싸게 처리하지만, 정확한 global recall에서는 손해를 볼 수 있다. Solar Open 2는 둘을 섞어 **긴 문맥 처리의 정확도와 비용을 함께 잡는 설계**를 택했다. CTO 설명에서도 이 조합으로 메모리와 computation을 약 1/4 수준까지 줄이는 점을 강조했다.
...
Solar Open 2는 softmax layer에 명시적 position signal을 넣지 않는다. 대신 token order와 long-range information을 linear attention의 sequential state가 들고 간다. 리포트 표현으로는 RoPE extrapolation limit과 softmax position-learning bottleneck을 함께 피하려는 설계다. 라이브에서 이활석 CTO는 “모델 스스로가 positioning을 익히게끔” 만들려 했다고 설명했다. 위치 정보를 사람이 정한 방식으로 박아 넣는 대신, linear state가 token order를 익히게 한 셈이다.
...
논문적인 디테일처럼 보이지만, agent 관점에서는 꽤 현실적이다. 긴 작업에서는 초반 state에 잘못 들어간 정보가 뒤까지 누적된다. Linear-heavy 구조라면 state가 “잊고 고치는 능력”을 가져야 한다.

CTO 설명에서 좋았던 대목은 여기다. 그는 negative eigenvalue를 둔 이유를 “이전에 쓴 것을 지우거나, 원래는 안 됐던 상황까지 학습되게 만들고 싶었다”는 취지로 설명했다. 긴 context를 많이 담는 저장소로만 보지 않고, state가 틀린 정보를 지우고 수정해야 한다고 본 것이다. 그는 이론적으로는 더 긴, 거의 무한대에 가까운 context도 가능한 구조라고 말하면서도, 실제 성능과 GPU 비용은 별도 문제라고 선을 그었다. “1M 지원”은 마케팅 문구가 될 수 있지만, 1M에서 쓸 만한 성능을 유지하는 것은 전혀 다른 문제다.

## Solar Open 1에서 2.3%만 가져왔다

또 하나 봐야 할 부분은 initialization이다. Solar Open 2는 Solar Open 1에서 출발했지만, 전체를 그대로 키운 모델이 아니다. 아키텍처가 크게 바뀌었기 때문에 호환되는 뼈대만 가져왔다.
...
라이브 설명도 현실적이었다. Solar Open 1의 결과를 “그냥 다 버리는 게 아니라” 최대한 활용하고 싶었지만, attention 구조와 RoPE 사용 여부가 바뀌어 direct mapping이 어려웠다고 한다. 그래서 이식 가능한 weight와 remapping 가능한 weight를 실험으로 찾아냈고, 결과적으로 250B 중 약 5.7B만 가져왔다. CTO는 이 기술을 다음 세대 Solar Open 3로 갈 때도 이어갈 수 있는 초기화 기술로 설명했다. 모델 재사용은 단순 CPT가 아니라, 아키텍처가 바뀐 상태에서 무엇을 옮길 수 있는지 찾는 engineering problem이 된 셈이다.
...
여기서 한 가지 흥미로운 고백도 있다. Length expansion 단계에서는 데이터 분포 변화 때문에 전체 성능이 약간 떨어졌고, Upstage는 네 개 중간 checkpoint를 merge해 최종 pre-trained model을 만들었다. 라이브에서도 같은 맥락을 설명했다. 1M context 데이터를 넣으면 긴 문맥 능력은 올라가지만 4K 같은 짧은 context 성능은 떨어질 수 있고, 이를 줄이려고 마지막에 여러 checkpoint를 merge했다는 것이다. 1M context를 붙이는 일은 길이만 늘리는 작업이 아니다. 기존 능력과 긴 문맥 능력 사이의 trade-off를 다루는 일이다.
...
문제는 specialist를 따로 잘 키우는 것과 하나의 모델로 합치는 것이 다르다는 점이다. teacher가 만든 trace만 SFT하면 student가 자기 실수로 흘러간 state에서 어떻게 복구해야 하는지 배우기 어렵다. 그래서 MOPD는 student가 직접 rollout한 trajectory 위에서, 해당 prompt에 routing된 specialist teacher의 full-vocabulary distribution을 따라가게 한다. 쉽게 말하면, **학생이 자기가 실제로 헤매는 길 위에서 선생님의 다음 선택 분포를 배운다.**
...
이 방식이 agent 모델에서 중요한 이유는 분명하다. Agent는 정답 문장 하나를 생성하는 모델이 아니라, 상태가 계속 바뀌는 환경에서 움직이는 모델이다. coding agent는 repo를 수정하고 테스트를 돌린다. officework agent는 xlsx/docx/pptx/pdf를 만들고, 숫자와 출처를 맞춘다. tool-use agent는 MCP 환경에서 state를 바꾼다. 이런 작업은 결과만 보고 학습하기 어렵다. trajectory 품질과 self-checking이 중요하다.

리포트의 fully asynchronous RL 설명도 이 맥락이다. Agent rollout은 길이가 들쭉날쭉하다. 동기식으로 처리하면 제일 느린 rollout을 기다리느라 GPU가 놀게 된다. Upstage는 actor와 trainer를 분리하고, stale token을 관리하며, 긴 trajectory를 버리지 않고 salvage하는 방식으로 agentic RL의 throughput 문제를 다뤘다. 이 대목은 모델 논문보다 agent harness 운영 보고서에 가깝게 읽힌다.

라이브에서 이활석 CTO는 이 부분을 현장 회고처럼 말했다. 처음 synchronously 학습을 돌렸을 때 GPU utilization이 약 20% 수준이었다고 한다. 나머지 80%가 노는 구조라면 대규모 RL에서는 답이 없다. 그래서 asynchronous로 갈 수밖에 없었고, 끝나는 시점도 reward 성격도 다른 trajectory들을 어떻게 묶어 training batch로 넣느냐가 핵심 문제가 되었다. 그는 “골든으로 다 잘되는 강화학습”은 거의 불가능에 가깝고, 각 회사의 인프라 상황에 맞는 방법을 찾아야 한다고 했다. Solar Open 2의 post-training은 알고리즘 하나가 아니라 GPU·CPU·가상환경·verifier·batching을 함께 맞춘 시스템 작업이었다.
...
이활석 CTO 설명을 들으면 Ko-GDPval의 의미가 더 분명해진다. 그는 RL에서 가장 큰 관문으로 **verifiable한 데이터를 다양하고 대량으로 뽑아내는 데이터 생성 파이프라인**을 꼽았다. 코딩 agent라면 실제로 코드를 고치고 테스트로 검증하는 환경이 필요하다. officework라면 문서나 스프레드시트, 발표자료가 제대로 만들어졌는지 확인하는 verifier가 필요하다. 그래서 GPU뿐 아니라 CPU와 실행 환경도 많이 필요하고, 구성원들이 많이 투입된 영역 중 하나가 agent가 뛰어놀 환경 구축이었다고 설명했다.

특히 officework 쪽은 OfficeVerse라는 내부 파이프라인과 이어진다. 문서 업무 타입을 늘리고, HWP 같은 한국 업무 포맷까지 추가하는 식으로 환경을 고도화하고 있다는 언급도 있었다. 이 대목이 한국어 LLM의 진짜 난이도를 보여준다. 한국어를 잘 말하는 것과 한국 회사의 문서·숫자·파일 형식·업무 관행을 맞추는 것은 다르다.
...
이 대목은 오히려 신뢰도를 조금 올린다. “모든 것을 이긴다”가 아니라, 어디서 강하고 어디에 과제가 남았는지 드러난다. Upstage도 conclusion에서 남은 gap을 repository/terminal-level software engineering, officework deliverable의 fine-grained numerical precision으로 적었다. 둘 다 더 강한 verification과 self-checking behavior가 필요한 영역이다.
...
다만 여기서 판이 조금 바뀌었다. Nota AI가 [Solar-Open2-250B-Nota-INT4-GlobalPruned](https://huggingface.co/nota-ai/Solar-Open2-250B-Nota-INT4-GlobalPruned)를 공개했다. Solar Open 2를 **W4A16 weight-only INT4**로 양자화하고, MoE expert를 전역 중요도 기준으로 pruning한 버전이다. 균등하게 expert를 자르는 방식이 아니라, 네트워크 전체에서 중요한 expert를 layer별로 다르게 남기는 global pruning을 쓴다.

숫자가 꽤 중요하다. Nota 모델카드 기준 weight footprint는 **117.8GB**까지 줄었다. unpruned INT4 모델은 2장에 올리기 어려워 4×H100으로 넘어가야 하지만, 이 globally pruned 모델은 **2×H100**에 올라간다. 예시 서빙 명령도 vLLM 기준 `--tensor-parallel-size 2`, `--max-model-len 131072`로 제시되어 있다. upstream vLLM이 per-layer variable expert count를 아직 직접 지원하지 않기 때문에 repository의 `patch/solar_open2.py`를 vLLM 모델 정의에 덮어써야 한다는 조건은 있다.

성능 손실도 생각보다 작다. 모델카드는 세 개 고난도 reasoning benchmark 평균에서 unpruned INT4가 83.99, Nota global-pruned INT4가 **83.39**, 같은 비율의 uniform pruning이 78.79라고 적는다. 정확히 말하면 “250B 모델을 아무 데서나 가볍게 돌린다”는 이야기는 아니다. 하지만 **회사·학교·랩의 in-house GPU 서버에서 한국어 agent model을 직접 서빙해볼 수 있는 선**까지 내려온 것은 맞다. 이 지점이 오픈웨이트의 현실적인 의미다. API로만 쓰는 모델이 아니라, 하우스 안의 데이터·문서·업무 시스템 곁에 붙여볼 수 있는 모델이 된다.

그럼에도 의미가 있다. 한국어권 오픈웨이트 모델이 단순 챗봇 품질 경쟁을 넘어, agent workflow와 office deliverable까지 정면으로 잡기 시작했다는 신호이기 때문이다.
...
이 흐름은 “큰 모델을 만들었다”보다 구체적이다. 특정 언어권의 sovereign model이 agent 시대에 어디를 파야 하는지 보여준다. native-language tokenizer, 실제 업무 데이터, 도구 호출 환경, deliverable 검증, 긴 trajectory 학습. 이 다섯 가지가 함께 가야 한다.
