...
공식 모델카드, Upstage 블로그, 34쪽짜리 [Solar Open 2 Technical Report](https://huggingface.co/upstage/Solar-Open2-250B/blob/main/Solar_Open_2_Tech_Report.pdf), 그리고 조코딩 채널의 [Solar Open Weight Day 라이브](https://www.youtube.com/live/6XX-yR3qomM) 자막을 같이 읽으면 그림이 더 선명해진다. 특히 6분 23초부터 이어지는 Upstage 이활석 CTO 설명은 테크니컬 리포트의 딱딱한 문장을 “왜 그렇게 설계했는가”로 풀어준다. 이 모델의 재미있는 지점은 벤치마크 숫자 하나가 아니다. **아키텍처, 데이터, post-training, agent scenario가 모두 “긴 작업을 끝까지 수행하는 모델” 쪽으로 정렬되어 있다.**
...
이활석 CTO는 라이브에서 이 선택을 더 직설적으로 설명했다. Solar Open 1은 all-softmax attention이었고, Solar Open 2에서는 softmax와 linear attention을 “3대 1”로 섞었다고 한다. 자동자막에는 순서가 조금 흔들리지만, 리포트 기준으로는 3개의 linear attention마다 1개의 softmax attention이 들어가는 구조다. 이유는 단순하다. softmax attention은 긴 범위에서 정확도는 좋지만 메모리와 연산량이 길이에 따라 크게 늘어난다. linear attention은 비용은 덜 늘지만 long-context 정확도 보장이 약하다. 그래서 둘을 섞어 **긴 문장 대응의 정확도와 비용을 동시에 잡는 설계**로 갔다. CTO 설명에서는 이때 메모리·컴퓨테이션 소비가 약 1/4 수준으로 떨어진다는 점을 강조했다.
...
Solar Open 2는 softmax layer에 명시적 position signal을 넣지 않는다. 대신 token order와 long-range information을 linear attention의 sequential state가 들고 가게 한다. 리포트 표현으로는 RoPE extrapolation limit과 softmax position-learning bottleneck을 함께 피하려는 설계다. 라이브에서 이활석 CTO는 “모델 스스로가 positioning을 익히게끔” 만들려 했다고 설명했다. 즉 위치 정보를 사람이 정한 방식으로 박아 넣는 대신, linear state가 token order를 학습하게 만드는 쪽이다.
...
CTO 설명에서 좋았던 대목은 여기다. negative eigenvalue를 둔 이유를 “이전에 쓴 것을 지우거나, 원래는 안 됐던 상황까지 학습되게 만들고 싶었다”는 취지로 설명한다. 긴 context를 단순히 많이 담는 저장소로 보는 것이 아니라, state가 틀린 정보를 지우고 수정할 수 있어야 한다는 관점이다. 그래서 그는 이론적으로는 더 긴, 거의 무한대의 context도 가능한 구조라고 말하면서도, 실제로는 성능이 나오느냐와 GPU가 받쳐주느냐가 별도 문제라고 선을 그었다. 이 균형감이 중요하다. “1M 지원”은 마케팅 문구지만, 1M에서 쓸 만한 성능을 유지하는 것은 전혀 다른 문제다.
...
라이브 설명에서는 이 부분도 꽤 현실적으로 나온다. Solar Open 1 결과를 “그냥 다 버리는 게 아니라” 최대한 활용하고 싶었지만, attention 구조와 RoPE 사용 여부가 바뀌었기 때문에 direct mapping이 어려웠다는 것이다. 그래서 이식 가능한 weight와 remapping 가능한 weight를 실험으로 찾아냈고, 결과적으로 250B 중 약 5.7B만 가져왔다. CTO는 이 기술을 다음 세대 Solar Open 3로 갈 때도 이어갈 수 있는 축적된 초기화 기술로 설명했다. 모델 재사용이 단순 CPT가 아니라, 아키텍처가 바뀐 상태에서 무엇을 옮길 수 있는지 찾는 engineering problem이 된 셈이다.
...
여기서 한 가지 흥미로운 고백도 있다. Length expansion 단계에서는 데이터 분포 변화 때문에 전체 성능이 약간 떨어졌고, Upstage는 네 개 중간 checkpoint를 merge해 최종 pre-trained model을 만들었다. 라이브에서도 비슷한 설명이 나온다. 1M context 데이터를 넣으면 긴 문맥 능력은 올라가지만 4K 같은 짧은 context 성능은 떨어질 수밖에 있고, 그 완화를 위해 마지막에 여러 checkpoint를 merge했다는 것이다. 1M context를 붙이는 일은 단순히 길이를 늘리는 문제가 아니라, 기존 능력과 긴 문맥 능력 사이의 trade-off를 다루는 작업이었다는 뜻이다.
...
라이브에서 이활석 CTO는 이 부분을 거의 현장 회고처럼 말했다. 처음에 synchronously 학습을 돌렸더니 GPU utilization이 약 20% 수준이었다고 한다. 나머지 80%가 놀고 있는 구조라면 대규모 RL에서는 답이 없다. 그래서 asynchronous로 갈 수밖에 없었고, 문제는 끝나는 시점도 다르고 reward 성격도 다른 trajectory들을 어떻게 묶어 training batch로 넣느냐가 되었다. 그는 “골든으로 다 잘되는 강화학습”은 거의 불가능에 가깝고, 각 회사의 인프라 상황에 맞는 방법을 찾아야 한다고 했다. 이 말이 좋다. Solar Open 2의 post-training은 알고리즘 하나라기보다, GPU·CPU·가상환경·verifier·batching을 같이 맞춘 시스템 작업이었다.
...
이활석 CTO 설명을 들으면 Ko-GDPval의 의미가 더 분명해진다. 그는 RL에서 가장 큰 관문이 **verifiable한 데이터를 다양하고 대량으로 뽑아내는 데이터 생성 파이프라인**이라고 했다. 코딩 agent라면 실제로 코드를 고치고 테스트로 검증하는 환경이 필요하고, officework라면 문서나 스프레드시트, 발표자료가 제대로 만들어졌는지 확인하는 verifier가 필요하다. 그래서 GPU뿐 아니라 CPU와 실행 환경도 많이 필요하고, 구성원들이 가장 많이 투입된 영역 중 하나가 agent가 뛰어놀 환경 구축이었다고 설명했다.

특히 officework 쪽은 OfficeVerse라는 내부 파이프라인과 이어진다. 문서 업무 타입을 늘리고, HWP 같은 한국 업무 포맷까지 추가해보는 식으로 환경을 계속 고도화하고 있다는 언급도 있었다. 이건 한국어 LLM의 진짜 난이도를 잘 보여준다. 한국어를 잘 말하는 것과 한국 회사의 문서·숫자·파일 형식·업무 관행을 맞추는 것은 다르다.
