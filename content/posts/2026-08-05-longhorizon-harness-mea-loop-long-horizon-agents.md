...
4. 결과 숫자. WeaveBench에서 Qwen 3.7-Plus가 51.8%→80.7%. Terminal-Bench 2.1에서 69.7%→77.2%. OSWorld 2.0에서 2.8%→8.3%로 binary completion이 3배 가까이 늘었음. Claude Opus 4.7을 쓰면 OSWorld 서브셋에서 20.6%→35.3%임. 약한 모델이든 강한 모델이든 하네스 개선이 일관되게 더해짐.
...
7. 적용 범위의 경계도 명확함. 환경 상태가 파일, 바이너리, 메타데이터로 검증 가능하면 효과가 큼. 문서 편집, 빌드, 시스템 관리가 대표적임. 반면 숨겨진 성능 임계값이나 비디오 시각 정밀도가 핵심인 작업에선 한계가 있고 MTEB, data-science, video-processing 태그에선 오히려 하락이 있었음.
