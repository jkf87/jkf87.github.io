...
1. 배경. TIR(도구 통합 추론) 에이전트는 하나의 작업에 수십 번 도구를 호출함. 주류인 GRPO는 궤적 끝에 보상 하나를 주고 모든 토큰에 동일하게 할당함. 올바른 호출과 잘못된 호출이 같은 크레딧을 받는 것. 턴 단위로 좋았는지 나빴는지 구분이 안 됨.

2. 기존 온폴리시 자기증류(OPSD)의 한계도 명확함. teacher에게 정답이나 참조 궤적을 privileged context로 주는데, 도구 호출 한 번이 환경을 바꿔버려서 참조 경로와 실제 경로가 금방 갈라짐. 상태 정렬이 깨진다는 것.
...
5. 다중 lookahead 합의도 눈여겨볼 것. 1, 2, 3턴 teacher 세 개 중 다수표로 방향을 정하고 그 방향에 동의하는 것 중 가장 강한 신호를 선택함. 그리고 GRPO advantage의 부호는 유지하고 크기만 변조함. 최적화 방향을 안 해치면서 신호를 조율하는 것 — 기존 파이프라인에 drop-in으로 붙는 이유.

6. 결과. Qwen3-4B/8B로 FTRL, BFCL, ToolHop 세 벤치마크에서 전부 기존 최고를 넘김. 특히 BFCL의 Long Context와 Miss Parameter에서 큰 폭 개선 — 여러 턴에 걸친 정확한 크레딧 할당이 필요한 문제들에서 효과가 크다는 것.
