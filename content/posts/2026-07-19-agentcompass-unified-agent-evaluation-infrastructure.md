...
5. 궤적 분석이 진짜 자산임. 추론 과정·도구 호출·환경 피드백·토큰 소비·지연 시간이 구조화되어 기록되고, 플러거블 분석기가 출력 잘림·지연 스파이크·반복 생성 루프·리워드 해킹 패턴을 자동 플래깅함. 실제로 특정 모델이 평가 기준 허점을 이용해 고득점한 패턴을 궤적 분석으로 잡아냈음. 스칼라 메트릭만 봤으면 영영 몰랐을 것임.
...
9. 문제제기. 3축 분리가 만능은 아님. 하네즈별 특성(프롬프트 포맷, 상태 관리 방식)이 성능에 섞여 들어가는 걸 완전히 통제하진 못함. 그리고 벤치마크 20개를 돌리는 비용 자체가 만만치 않아서 전수 평가는 연구소가 아니면 어려움. 내 용도는 필요한 3-4개 벤치마크 + 하네스 고정 비교가 현실적임.
