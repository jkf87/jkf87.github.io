...
2. 스킬의 첫 비용이 description임. progressive disclosure(모델이 필요할 때만 단계적으로 내용을 공개하는 스킬 설계 방식)의 첫 층인 description은 모든 모델 호출 컨텍스트에 항상 들어가서 매번 토큰 비용을 냄. 그래서 에세이가 아니라 지시어여야 함. "chat application을 만들 때 Interactions API를 사용하라"가 "Interactions API는 multi-chat에 권장됩니다"보다 낫다는 것. 배경지식이 아니라 행동을 바꾸는 지시를 줘야 함.
...
4. AI가 만든 스킬은 성능을 떨어뜨릴 수 있다는 경고가 핵심임. SkillsBench 1.1(에이전트 스킬 활용 능력을 재는 벤치마크)에서 스킬은 평균 약 15% 향상이 있었지만 사람이 쓴 스킬이 가장 좋았고, AI 생성 스킬엔 no-op(읽어도 행동에 아무 변화를 만들지 않는 내용)이 많이 들어감. "명확하고 고품질의 코드를 작성하라" 같은 문장은 틀리지 않았지만 모델이 원래 할 말이라 행동을 바꾸지 않음. 행동을 바꾸지 않는 문장은 비용이라는 것 — 좋은 스킬 문장은 아름다운 문장이 아니라 나쁜 선택지를 줄이는 문장임.
...
8. 필자가 바로 한 것. 가장 자주 불리는 스킬 하나를 골라 발동 프롬프트 3개, 비발동 프롬프트 2개, regex 체크, on/off ablation(같은 작업을 스킬 있음/없음으로 나눠 돌려 기여도를 재는 비교) 한 번. merche 스킬이 실제로 재작성 품질을 올리는지 아니면 모델이 이미 잘하는 건지 확인하는 게 첫 순서임. 스킬은 자산이지만 동시에 고정비라는 관점을 유지해야 함.
